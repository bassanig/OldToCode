import React, { useRef, useEffect } from 'react';
import { NavLink } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import conteudos from '../../../data/conteudos.json';
import TituloTexto from '../TipografiaPaginas/TituloTexto';
import fotoDados from '../../../assets/fotos/fundamentos/dados.png';
import fotoBateria from '../../../assets/fotos/conteudos/bateriaVolta2.png';
import useOutsideClick from '../../Hooks/useOutsideClick';

const FundamentosMenu = ({ setShowFundamentos }) => {
  const { t, i18n } = useTranslation();
  const menuRef = useRef(null);

  useOutsideClick(menuRef, () => setShowFundamentos(false));

  useEffect(() => {
    document.body.style.overflow = 'hidden';
    return () => { 
      document.body.style.overflow = 'auto';
    };
  }, []);

  const fundamentosKeys = Object.keys(conteudos).filter(key =>
    key.startsWith('fundamentos-'),
  );

  return (
    <div
      ref={menuRef}
      className="w-full bg-gray-50 absolute z-30 dark:bg-dark max-lg:hidden animate-slideDown py-8 flex justify-center"
    >
      <div className=" mx-auto flex max-w-fit gap-8">
        <div className="flex flex-col justify-start">
          <h1 className="text-5xl text-vermelho font-serif font-bold mb-4 dark:text-amarelo-dark ">
            {t('header.nav.fundamentals')}
          </h1>
          <ul className="flex flex-col gap-4 py-2">
            {fundamentosKeys.map(key => (
              <li key={key}>
                <NavLink
                  to={`/conteudo/${key}`}
                  className="block px-1 py-2 text-lg text-dark dark:text-gray-200 hover:text-amarelo dark:hover:text-amarelo"
                  onClick={() => setShowFundamentos(false)}
                >
                  {conteudos[key][i18n.language]?.titulo ||
                    conteudos[key]['pt'].titulo}
                </NavLink>
              </li>
            ))}
          </ul>
        </div>
        <div className="flex flex-col relative px-6">
          <div className="flex items-center justify-between pb-4">
            <h1 className="text-xl text-vermelho pt-3 dark:text-amarelo-dark">
              {t('others')}
            </h1>
            <button
              onClick={() => setShowFundamentos(false)}
              className=" cursor-pointer dark:hover:bg-vermelho hover:bg-amarelo-dark trasition duration-150 right-0 w-9 h-9 rounded-full border dark:border-amarelo-dark border-vermelho flex items-center justify-center"
            >
              <span className="absolute w-5 h-0.5 bg-vermelho rounded-2xl -rotate-45 dark:bg-amarelo-dark"></span>
              <span className="absolute w-5 h-0.5 bg-vermelho rounded-2xl rotate-45 dark:bg-amarelo-dark"></span>
            </button>
          </div>
          <div className="flex gap-4 ">
            <NavLink
              onClick={() => setShowFundamentos(false)}
              to="/conteudo/alessandro-volta-bateria"
              className="overflow-hidden block max-w-[400px] rounded-2xl relative border-gray-50"
            >
              <img
                src={fotoBateria}
                alt=""
                className="object-cover h-full w-full"
              />
              <div className="absolute flex flex-col gap-2 bg-gray-50 dark:bg-dark bottom-0 left-0 w-full px-6 py-4 border-6 border-gray-50 dark:border-dark">
                <h1 className="text-2xl dark:text-gray-50">
                  {t('tools.battery.title')}
                </h1>
                <span className="text-md dark:text-gray-50">
                  {t('tools.battery.description')}
                </span>
              </div>
            </NavLink>
            <NavLink
              onClick={() => setShowFundamentos(false)}
              to="/conteudo/fundamentos-dados"
              className="overflow-hidden block max-w-[400px] rounded-2xl relative border-gray-50"
            >
              <img
                src={fotoDados}
                alt=""
                className="object-cover h-full w-full"
              />
              <div className="absolute flex flex-col gap-2 bg-gray-50 dark:bg-dark bottom-0 left-0 w-full px-6 py-4 border-6 border-gray-50 dark:border-dark">
                <h1 className="text-2xl dark:text-gray-50">
                  {t('home.logicFundamentals.data.title')}
                </h1>
                <span className="text-md dark:text-gray-50">
                  {t('home.logicFundamentals.data.description')}
                </span>
              </div>
            </NavLink>
          </div>
        </div>
      </div>
    </div>
  );
};

export default FundamentosMenu;
